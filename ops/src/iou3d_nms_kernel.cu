...
---------------------------------------------------------------------------
modified by DenseBEV: add quick reject to NMS kernel 
---------------------------------------------------------------------------
*/
...
__device__ inline bool quick_reject(const float* box_a, const float* box_b, float threshold) {
    float center_dist_sq = (box_a[0] - box_b[0]) * (box_a[0] - box_b[0]) +
                           (box_a[1] - box_b[1]) * (box_a[1] - box_b[1]);

    float max_radius = fmaxf(box_a[3], box_a[4]) + fmaxf(box_b[3], box_b[4]);
    return center_dist_sq >= max_radius * max_radius;
}
...
__global__ void nms_kernel(const int boxes_num, const float nms_overlap_thresh,
    ...
    if (threadIdx.x < row_size) {
        ...
        for (i = start; i < col_size; i++) {
            if (!quick_reject(cur_box, block_boxes + i * 7, 1.2f)){
